...
BLUE = [0,50,98]/255;
GOLD = [253,181,21]/255;
h1 = errorbar(nObs,mean(DQ(:,:,1),2),sem(DQ(:,:,1),2),'color',BLUE,'linewidth',3);
h2 = errorbar(nObs,mean(DQ(:,:,2),2),sem(DQ(:,:,2),2),'color',GOLD,'linewidth',3);
...
h5 = errorbar(1,mean(DQ(2,:,1),2)-mean(DQ(1,:,1),2),sem(DQ(2,:,1),2),'*g');
h6 = errorbar(1,mean(DQ(2,:,2),2)-mean(DQ(1,:,2),2),sem(DQ(2,:,1),2),'*r');
legend([h1,h3,h2,h4,h5,h6],'1-lightbulb','1-lightbulb optimal','3-bandit','3-bandit optimal','1-lightbulb VOC1','3-bandit VOC1','location','southeast')
