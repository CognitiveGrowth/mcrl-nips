...
def extract_csv(csv):
    df = pd.read_csv(csv)
    wids = df['Amazon Worker ID']
    return [w for w in wids if w.startswith('A')]
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "-qualification",
        default='3JC3H4XMU8HG56C28J5BT7S25MJ3CG',  # Web of Cash Compensation
    ...
    new_quals = get_quals()
    print("\n%d workers with qualification %s:" % (len(new_quals), args.qualification))
