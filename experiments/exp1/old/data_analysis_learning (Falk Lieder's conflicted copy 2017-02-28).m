...
ylabel('relative reward','FontSize',18)
xlim([-.2 16.2])
...
%     rm = fitrm(t,'t1-t20 ~ Group','WithinDesign',Time);
    ...
    y3=[y1,y2];
    mdl_both = fitnlm(X3,y3(:),'y ~ (1-b1)*sigmoid(b2+(b3+b4*x2)*x1)',[0.01;0.01;0.25;0.2])
...
xlabel('trial','fontsize',18)
ylabel('delay after 1^{st} move (sec)','fontsize',18)
...
legend('feedback','no feedback','location','southeast')
xlabel('trial','fontsize',18)
ylabel('% optimal routes','fontsize',18)
...
if DO_STATS
    ...
    y3=[y1,y2];
    mdl_both = fitnlm(X3,y3(:),'y~ (1-b5+b6*x2)*sigmoid((b1+b2*x2)*x1+b3*x3+b4*x2)' ,[0.01;0.01;0.25;0.1;0.2;0.1])
end
...
legend('feedback','no feedback','location','southeast')
xlabel('trial','fontsize',18)
ylabel({'# clicks' 'before 1^{st} move'},'fontsize',18)
...
if DO_STATS
    ...
    mdl_both = fitnlm(X3,y3_rel(:),'y~ (1-b1)*sigmoid((b2+b3*x2)*x1+b4*x3)' ,[0.01;0.01;0.25;0.2])
end
