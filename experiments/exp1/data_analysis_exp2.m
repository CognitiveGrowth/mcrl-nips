...
% 2B conditions: 0: no FB, no break; 1: no FB, 10min break; 2: FB, no break; 3: FB, 10min break
idx0 = [data.condition]==0;
idx1 = [data.condition]==1;
idx2 = [data.condition]==2;
idx3 = [data.condition]==3;
if strcmp(experiment_version,'2B')
    legendStr = {'feedback, 10min break','feedback, no break','no feedback, 10min break','no feedback, no break'};
    legendOrd = '[h3,h2,h1,h0]';
    colorOrd = {'BLUE','BLUE','GOLD','GOLD'};
    linestyleOrd = {'--','-','--','-'};
else
    legendStr = {'feedback, 10min break','feedback, no break','no feedback, 10min break','no feedback, no break'};
    legendOrd = '[h3,h2,h1,h0]';
...
figure; hold on;
y0 = [data(idx0).relative_reward];
y1 = [data(idx1).relative_reward];
y2 = [data(idx2).relative_reward];
y3 = [data(idx3).relative_reward];
...
%     rm = fitrm(t,'t1-t20 ~ Group','WithinDesign',Time);
    close all hidden
    conds = [zeros(size(y0(:))); ones(size(y1(:))); 2*ones(size(y2(:))); 3*ones(size(y3(:)))];
